(() => {
  ...
  function buildPatternAnalysisContent(data, name) {
    ...
    note.className = "note";
    note.innerHTML =
      `以下是對<strong>${name}</strong>最近K線的技術型態辨識與量價分析——全部是對「已經發生的K棒」做客觀分類，` +
    ...
    if (data.volume) {
      ...
      const items = [
        ...
        ["近5日均量 vs 近20日均量", `${v.vol_ratio_pct > 0 ? "+" : ""}${v.vol_ratio_pct}%（${v.vol_trend}）`],
      ];
      for (const [label, value] of items) {
        const item = document.createElement("div");
        item.className = "ref-item";
  ...
  function renderReversalScreenerTable() {
    ...
    if (!rows.length) {
      reversalScreenerBody.innerHTML = `<tr><td colspan="22" class="empty">沒有符合條件的股票</td></tr>`;
      pagination.style.display = "none";
      return;
    }
  ...
  async function runReversalScreener() {
    ...
    reversalScreenerBody.innerHTML = `<tr><td colspan="22" class="empty">掃描中…</td></tr>`;
    try {
      ...
      reversalScreenerBody.innerHTML = `<tr><td colspan="22" class="empty">掃描失敗</td></tr>`;
      console.error(err);
    } finally {
      btn.disabled = false;
    }
